...
time = (0 : total_points-1) * dt;

q = [1 0 0 0]';

accel_ideal = zeros(3, total_points);
gyro_ideal = zeros(3, total_points);
euler_ideal = zeros(3, total_points);
accel = zeros(3, total_points);
gyro = zeros(3, total_points);
...
for static_idx = 1:static_points
    ...
    for i = 1:rotation_points
        ...
        euler_ideal(:, current_idx) = quat2euler(q);
...
accel = inv(T_a) * inv(K_a) * accel_ideal + b_a + accel_noise;

% --------------------------修改处--------------------------
% 按文献模型修正陀螺仪误差施加顺序：ω^S = (T^g K^g)⁻¹ ω^O - (b^g + 漂移) + 噪声
gyro_noise = gyro_noise_std * randn(3, total_points);
gyro_error_matrix = T_g * K_g;
gyro = inv(gyro_error_matrix) * gyro_ideal + (b_g + gyro_drift) + gyro_noise;
% ----------------------------------------------------------
...
plot3(accel(1,:), accel(2,:), accel(3,:), 'r.', 'MarkerSize', 2);
...
subplot(3,1,1);
plot(time, accel_ideal(1,:), 'b-', 'LineWidth', 0.8);
hold on; plot(time, accel(1,:), 'r-', 'LineWidth', 0.5);
xlabel('时间(s)'); ylabel('X轴加速度(g)'); grid on; legend('理想', '带误差');
subplot(3,1,2);
plot(time, accel_ideal(2,:), 'b-', 'LineWidth', 0.8);
hold on; plot(time, accel(2,:), 'r-', 'LineWidth', 0.5);
xlabel('时间(s)'); ylabel('Y轴加速度(g)'); grid on;
subplot(3,1,3);
plot(time, accel_ideal(3,:), 'b-', 'LineWidth', 0.8);
hold on; plot(time, accel(3,:), 'r-', 'LineWidth', 0.5);
xlabel('时间(s)'); ylabel('Z轴加速度(g)'); grid on;
sgtitle('加速度计各轴时序(888Hz，固定点停留10秒)');

figure('Color', 'w', 'Position', [100 300 1000 600]);
subplot(3,1,1);
plot(time, gyro_ideal(1,:), 'b-', 'LineWidth', 0.8);
hold on; plot(time, gyro(1,:), 'r-', 'LineWidth', 0.5);
xlabel('时间(s)'); ylabel('X轴角速度(dps)'); grid on; legend('理想', '带误差');
subplot(3,1,2);
plot(time, gyro_ideal(2,:), 'g-', 'LineWidth', 0.8);
hold on; plot(time, gyro(2,:), 'r-', 'LineWidth', 0.5);
xlabel('时间(s)'); ylabel('Y轴角速度(dps)'); grid on;
subplot(3,1,3);
plot(time, gyro_ideal(3,:), 'b-', 'LineWidth', 0.8);
hold on; plot(time, gyro(3,:), 'r-', 'LineWidth', 0.5);
